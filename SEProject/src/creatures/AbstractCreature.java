...
public abstract class AbstractCreature implements ICreature {
	...
	/** Color of the creature proportional to energy*/
	...
	protected int cycleCountForDecreaseEnergy = 1;
	protected int cycleCountForIncreaseEnergy = 30;
	
	private final static double CYCLES_ENERGY_DIRECTION = 70;
	protected double energypointDirectionCycle = CYCLES_ENERGY_DIRECTION + 1;
	
	protected static final double MIN_SPEED = 3;
	protected static final double MAX_SPEED = 7;
	
	public static final int cyclesBeforeDecrease = 10;
	
	protected int energyDecreaseParam = -3;
	protected int energyIncreaseParam = 100;
	...
	protected boolean senseEnergyPoint()
	{
		if (energypointDirectionCycle < CYCLES_ENERGY_DIRECTION)
		{
			energypointDirectionCycle++;
			return false;
		}
		
		
		for (EnergyPoint k : environment.getEnergyPoints())
			if(distanceFromAPoint(k.getPoint()) < k.eventHorizon)
			{
				energypointDirectionCycle = 0;			
				return true;			
			}
	...
	protected double getEnergyPointDirection()
	{
		//step1: find closest energy point
		Iterable<EnergyPoint> energyPts = environment.getEnergyPoints();
		Iterator<EnergyPoint> x = energyPts.iterator();
		EnergyPoint ClosestEnergyPt = x.next();
		for (EnergyPoint k : energyPts)
			if(distanceFromAPoint(k.getPoint()) < distanceFromAPoint(ClosestEnergyPt.getPoint()))
				ClosestEnergyPt = k;
		
		
		//step2: change direction towards closest energy pt
		double b = 0;
		boolean flag = false;

		if (ClosestEnergyPt.getPoint().getX() != position.getX()) {
			b = Math.atan((double)(position.getY() - ClosestEnergyPt.getPoint().getY()) / (double)(position.getX() - ClosestEnergyPt.getPoint().getX()));
		} else if (position.getY() < ClosestEnergyPt.getPoint().getY()) {
			b = Math.PI / 2;
			flag = true;
		} else if (position.getY() > ClosestEnergyPt.getPoint().getY()){
			b = -Math.PI / 2;
		...
		if(!flag)
		{
			if (position.getX() > ClosestEnergyPt.getPoint().getX())
				b = (b + Math.toRadians(180));
			if (position.getX() == ClosestEnergyPt.getPoint().getX())
				if(position.getY() > ClosestEnergyPt.getPoint().getY())
					b = Math.toRadians(180);
				else
					b = Math.toRadians(0);
		}
	...
	final protected void changeEnergyLevel(int ch)
	{
		if (ch > 0)
			energy = Math.min(255, energy + ch);
		if (ch < 0)
			energy = Math.max(0, energy + ch);
		
		color = new Color(energy, 0, 255 - energy);
	}
	
	public void decreaseEnergy()
	{
		changeEnergyLevel(energyDecreaseParam);
	}
	
	public void increaseEnergy()
	{
		changeEnergyLevel(energyIncreaseParam);
	}
	...
	boolean closeToEnergyPt()
	{
		Iterable<EnergyPoint> pts = environment.getEnergyPoints();
		for (EnergyPoint p : pts)
		{
			if (distanceFromAPoint(p.getPoint()) < p.getInfluence())
				return true;
		}
		return false;
	}
